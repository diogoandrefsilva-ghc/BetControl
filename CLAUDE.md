# BetControl — guia para o assistente

App pessoal de controlo de apostas (casas e movimentos).
**Sem build, sem npm.** Site estático (GitHub Pages), PWA, + **Supabase (REST)**.

## Estrutura
- `index.html` — **ficheiro único** (~594 linhas): markup + CSS + JS tudo dentro. (Ainda NÃO dividido como o SplitBill/FestasBV.)
- `sw.js` — service worker (cache PWA).
- Não mexer: `db/`, ícones, `manifest.json`.
...
- **Supabase:** schema `apostas` (tabelas `casas`, `movimentos`). A chave no `<script>` é a **`anon` (pública por design)**, protegida por RLS + login. **Não é bug nem risco — não a "corrijas" nem a escondas.**
